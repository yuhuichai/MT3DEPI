...
cd ${dataDir}
for subj in 200306wholebrain.Subj03.tutorial; do
{
	subjDir=${dataDir}/${subj}
	cd ${subjDir}
	epi=${subjDir}/anat_wholebrain_mt.sft/mean.sub_d_dant.masked.denoised.nii
	...
	3dcalc -a ${subjDir}/anat_wholebrain_mt.sft/mean.sub_d_dant.nii \
		-b ${epiMask} -expr 'step(b)*a*step(a+0.1)*step(1-a)' \
		-prefix ${subjDir}/anat_wholebrain_mt.sft/mean.sub_d_dant.masked.nii \
		-overwrite

	DenoiseImage -d 3 -n Gaussian -i ${subjDir}/anat_wholebrain_mt.sft/mean.sub_d_dant.masked.nii \
		-o ${subjDir}/anat_wholebrain_mt.sft/mean.sub_d_dant.masked.denoised.nii

	transformFile=${subjDir}/anat_wholebrain_mt.sft/manual_align.txt # manual_align.txt is created mannually in ITK-SNAP with manual alignment
	outDir=${subjDir}/anat_wholebrain_mt.sft

	brainDir=${subjDir}/mp2rage
	...
	brainMask=${subjDir}/anat_wholebrain_mt.sft/mask.anat2brain_manual_aligned.nii
